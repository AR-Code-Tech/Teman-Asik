...
class _BusListDetailScreenState extends State<BusListDetailScreen> {
  ...
  void _locateRoute() {
    _googleMapController.animateCamera(CameraUpdate.newCameraPosition(
        CameraPosition(target: centerPos, zoom: 17)));
  }
  ...
  Future<List<RouteModel>> _getRoute() async {
    var url = Uri.parse(
        'http://68.183.227.243:3001/routes?rute=Rute%20Lyn%20${routeArgs.id.toUpperCase()}');
    ...
    setState(() {
      ...
      _polylines.add(p);
      centerPos =
          (routesLatLngList.length > 0) ? routesLatLngList.first : LatLng(0, 0);
    });
